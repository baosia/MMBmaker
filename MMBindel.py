...
ins_len = int(input('Enter desired length of MMBIR insertion (nt): '))      #Length of mmbir insertion desired..... CHANGE TO VARIABLE within a range
temp_finder = int(input('Enter desired distance to insertion (nt): '))   #Distance that template finder is expected to look.  ...Change to variable within a rage
...
def make_files():
    ...
    mmbir_list = [j if 'N' in j else j.replace(j[temp_finder:((temp_finder + ins_len) - 1)], h, 1) for (j, h) in zip(regions_list, insertions_list)]
    #For any region that doesn't contain an "N", this will replace sequence downstream of the template with the insertion
    #THIS VERSION REPLACES SEQUENCE (creates INDEL).
    #f2.write('\n'.join(mmbir_list))
    f5.write('\n'.join(mmbir_list))
